...
def main(j=None):
    ...
    for index, batch in enumerate(testloader):
        ...
        if args.dataset == 'DarkZurich':
            prob_int16 = output_prob * 65535
            prob_int16 = np.asarray(np.argmax(prob_int16, axis=2), dtype=np.uint16)
            prob_int16 = Image.fromarray(prob_int16)
            print(name)
            prob_int16.save('/home/jerry/xxz/DANNet/result/confidence/' + name[0].split('/')[-1])
        #####计算最大概率图
        if args.save_prob:
        ...
        if args.save_prob:
            ...
            plt.close()
        if args.dataset == 'DarkZurich':
            output.save('/home/jerry/xxz/DANNet/result/labelTrainIds/' + name)
