...
def main(args, j=None, checkpoint_dir=None):
    os.environ['CUDA_VISIBLE_DEVICES'] = '0'
    device = torch.device("cuda")


    # args = get_arguments()
    train_args = train_get_arguments()
    ...
    if j is not None:
        args.restore_from='./snapshots/{}/dannet'.format(checkpoint_dir)+str(j)+'.pth'
        args.restore_from_light='./snapshots/{}/dannet_light'.format(checkpoint_dir)+str(j)+'.pth'
        print(args.restore_from)

    saved_state_dict = torch.load(args.restore_from)
    model_dict = model.state_dict()
    saved_state_dict = {k: v for k, v in saved_state_dict.items() if (k in model_dict and k != 'cca.distance_weighted_h_M' and k != 'cca.distance_weighted_w_M')}
    ...
    testloader = data.DataLoader(zurich_night_DataSet(args.data_dir, args.data_list, set=args.set, use_lightM=args.use_lightM))
    ...
    for index, batch in enumerate(ACDCloader):
    # for batch in tqdm(ACDCloader):
        if index % 10 == 0:
            print('%d processd' % index)
        if index == args.eval_stop_at:
            print('We have prosessed {} images in dataset {}'.format(args.eval_stop_at, args.dataset))
            break

        if args.use_lightM:
            image, name, lightMask = batch['image_n'], batch['name'], batch['lightMask']
            lightMask = lightMask.to(device)
        else:
            image, name = batch['image_n'], batch['name']

        ACDClabel = batch['origin_label']
        print(torch.unique(ACDClabel))
        # exit()
        ...
        with torch.no_grad():
            ...
            if args.model == 'RefineNet':
                ...
                if args.use_lightM:
                    _, output2, features = model(enhancement, name=name, data_from='target', light_M=lightMask)
                elif args.dataset == 'cityscapes':
                    _, output2, features = model(image)
                else:
                    # _, output2, features = model(enhancement, name=name, data_from='source', light_M=None)
...
if __name__ == '__main__':
    ...
    for i in range(18, 19):
        j=(i+1)*1000
        main(args, j, args.checkpoint_dir)
        # mIou=compute_mIoU('/path/to/Dark_Zurich_val_anon/', './result/dannet_PSPNet', './dataset/lists')
        mIou=compute_mIoU('/path/to/Dark_Zurich_val_anon/', './result/dannet_PSPNet', './dataset/lists', dataset_name=args.dataset, compute_stopat=args.eval_stop_at)
