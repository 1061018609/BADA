...
from .feature_purity import FeatureSpaceRegionScore
from .feature_entropy import FeatureSpaceEntropy
from .feature_combine import FeatureSpacePurityEntropyScore
...
def PixelSelection(args, lightnet, seg_net, tgt_epoch_loader, class_features=None, i_iter=None):
    lightnet.eval()
    seg_net.eval()

    active_pixels = math.ceil(args.active_pixel_nums / len(args.active_select_iter_list) / (args.input_size * args.input_size) * (1920 * 1080))
    calculate_purity = SpatialPurity(in_channels=args.num_classes, size=2 * args.radius_k + 1).cuda()
    if args.feature_space_score_mode == 'only_purity':
        featurespace_region_score = FeatureSpaceRegionScore(args=args)
    elif args.feature_space_score_mode == 'only_entropy':
        featurespace_region_score = FeatureSpaceEntropy(args=args, classes_prototypes=class_features.objective_vectors)
    elif args.feature_space_score_mode == 'purity_and_entropy':
        featurespace_region_score = FeatureSpacePurityEntropyScore(args=args, classes_prototypes=class_features.objective_vectors)
    mask_radius = args.radius_k
    ...
    with torch.no_grad():
        for tgt_data in tqdm(tgt_epoch_loader):
            ...
            if args.model == 'RefineNet':
                ...
                if args.use_lightM:
                    _, tgt_out, features = seg_net(enhanced_image, data_from='target', light_M=lightMask)
                else:
                    _, tgt_out, features = seg_net(enhanced_image)
            ...
            for i in range(len(origin_mask)):
                ...
                one_hot = F.one_hot(pseudo_label, num_classes=args.num_classes).float()
                ...
                if args.feature_space_score_mode == 'only_purity':
                    features_score = featurespace_region_score(features, output)
                    score = ((purity+features_score)/2) * entropy
                elif args.feature_space_score_mode == 'only_entropy':
                    features_entropy = featurespace_region_score(features, output)
                    if i_iter >= args.prototypes_init_iter:
                        # score = purity * ((entropy+features_entropy)/2)
                        score = ((purity+features_entropy)/2) * entropy
                elif args.feature_space_score_mode == 'purity_and_entropy':
                    features_purity, features_entropy = featurespace_region_score(features, output)
                    if i_iter >= args.prototypes_init_iter:
                        score = ((purity+features_purity)/2) * ((entropy+features_entropy)/2)
                    else:
                        score = ((purity+features_purity)/2) * entropy
                else:
                    score = entropy * purity
        ...
        for tgt_data in tqdm(tgt_epoch_loader):
            ...
            if args.model == 'RefineNet':
                ...
                if args.use_lightM:
                    ...
                    _, tgt_out, _ = seg_net(enhanced_image)
...
# def RegionSelection(args, lightnet, seg_net, tgt_epoch_loader, class_features=None, i_iter=None):
...
#                 # name = tgt_data['name'][0]
#                 # features_score_heatmap = features_score.squeeze(0).squeeze(0).cpu().numpy()
#                 # sns.set()
#                 # sns.heatmap(features_score_heatmap)
#                 # plt.savefig('/home/jerry/xxz/DANNet/dataset/ACDC/gt_trainval/active_gt/night/output_visualization/features_score_heatmap/%s/%s_features_score.png' % (name.split('/')[0], name.split('/')[1]))
#                 # plt.close()
...
def Classification_difference_calculation(input_feature, seg_net, weights, batch_size):
    seg_net.eval()
