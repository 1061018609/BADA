...
class RCCLAModule(nn.Module):
    def __init__(self, in_channels, out_channels=512, num_classes=19, channels_down_ratio = 4):
        super(RCCLAModule, self).__init__()
        inter_channels = in_channels // channels_down_ratio
    ...
    def forward(self, x, recurrence=2, data_from=None, light_M=None):
        output = self.conva(x)
        for i in range(recurrence):
            output = self.cca(output, data_from=data_from, light_M=light_M)
        output = self.convb(output)

        output_features = self.bottleneck(torch.cat([x, output], 1))
...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes):
        ...
        if self.uselightconvs:
            self.lightconvslist = nn.Sequential(
                conv3x3(self.lightconv_inchannels, 32, stride=2),
                BatchNorm2d(32),
                ...
                BatchNorm2d(16),
                nn.Conv2d(16, 1, kernel_size=3, stride=2, padding=2, bias=False),
                BatchNorm2d(1),
                nn.Sigmoid())
        #####lightM conv layers
        ...
        self.head = RCCLAModule(2048, 512, num_classes)
    ...
    def forward(self, x, name=None, recurrence=2, data_from=None, light_M=None):
        ...
        # print(x.size(), light_M.size())
        if self.uselightconvs and data_from == 'target':
            light_M = self.lightconvslist(light_M)
        ...
        # print(x.size(), light_M.size())
        # exit()
        x, x_features = self.head(x, recurrence=recurrence, data_from=data_from, light_M=light_M)
